// frontend/apps/frontend/src/pages/MainPage.tsx
...
const MainPage: React.FC = () => {
  ...
  const handleUpdateSubmit = (productToUpdate: Product) => {
    fetch(`http://localhost:8080/product/${productToUpdate.id}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name: productToUpdate.name, description: productToUpdate.description })
    }).then(response => response.ok ? response.json() : null)
      .then(data => {
        if (data !== null) {
        ...
        <VerticalContainer>
          <div>
            ...
            {products.map(product => (
              <ReadContentBox content={product} key={`${product.id}-${product.name}-${product.description}`} />
            ))}
          </div>
        </VerticalContainer>
      </div>
      <div>
        <VerticalContainer>
          <div>
            <h2>Update</h2>
            {products.map(product => (
              <UpdateContentBox content={product} onSubmit={handleUpdateSubmit} key={`${product.id}-${product.name}-${product.description}`} />
            ))}
          </div>
        </VerticalContainer>
      </div>
      <div>
        <VerticalContainer>
          <div>
            <h2>Delete</h2>
            {products.map(product => (
              <DeleteContentBox content={product} onSubmit={handleDeleteSubmit} key={`${product.id}-${product.name}-${product.description}`} />
            ))}
          </div>
        </VerticalContainer>
      </div>
